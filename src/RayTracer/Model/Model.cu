...
__host__ __device__ Triangle::Triangle(float3 p[3], float3 norm[3]) {
	pos[0] = p[0];
	pos[1] = p[1];
	pos[2] = p[2];
	normal[0] = norm[0];
...
__host__ __device__ Triangle::Triangle(const float3 p[3], const float3 norm[3]) {
	pos[0] = p[0];
	pos[1] = p[1];
	pos[2] = p[2];
	normal[0] = norm[0];
...
__host__ __device__ Triangle::Triangle(float3 a, float3 b, float3 c, float3 norm[3]) {
	pos[0] = a;
	pos[1] = b;
	pos[2] = c;
	normal[0] = norm[0];
...
__host__ __device__ Triangle& Triangle::operator=(const Triangle& plus) {
...
//__host__  bool readFile(Mesh *obj, char * path) {
//	ifstream file(path);
//
...
//
//	}
//	cudaMalloc((void**)&(obj -> meshTable), sizeof(Triangle)*(vFace.size() + 1));
//	for (int i = 0; i < vFace.size(); i++) {
//		obj -> meshTable[i] = tempMesh[i];
//	}
//	obj -> number = vFace.size();
//	return true;
//}

__host__ __device__ bool  Mesh::hit(Ray r, IntersectRecord &colideRec) {
	//naive hit

...
public:
    ...
    CUDA_FUNC virtual float3 f(const float3 &wo, const float3 &wi) const = 0;
    //emited ray, incident ray, possibility for wi, the 2-D sample points between [0, 1]
    CUDA_FUNC virtual float3 sample_f(const float3 &wo, float3 *wi, float *pdf, const float2 &sample) const = 0;
...
public:
    ...
    CUDA_FUNC ~Lambertian() = default;
    CUDA_FUNC float3 f(const float3 &wo, const float3 &wi)const  override;
    CUDA_FUNC float3 sample_f(const float3 &wo, float3 *wi, float *pdf, const float2 &) const override;

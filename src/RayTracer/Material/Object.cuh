...
public:
    ...
    CUDA_FUNC float3 f(float3 normal, float3 tangent, const float3 &wo, const float3 &wi, float3 color = make_float3(-1.0f,-1.0f,-1.0f)) const;
    //emited ray, incident ray, possibility for wi, the 2-D sample points between [0, 1]
    CUDA_FUNC  float3 sample_f(float3 normal, float3 tangent, const float3 &wo, float3 *wi, float *pdf, const float2 &sample, float3 color = make_float3(-1.0f, -1.0f, -1.0f)) const;

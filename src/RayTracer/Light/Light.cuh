...
public:
    ...
    CUDA_FUNC virtual float3 lightIllumi(Scene &scene, IntersectRecord &ref, Ray *wi, float2 sample = make_float2(0.0f, 0.0f)) const = 0;
};
...
public:
    ...
    CUDA_FUNC float3 lightIllumi(Scene &scene, IntersectRecord &ref, Ray *wi, float2 sample = make_float2(0.0f, 0.0f)) const override;
private:
    float3 pos, illum;
};
